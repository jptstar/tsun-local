...
Git tags use a `v` prefix. For example, release `1.3.2` uses tag `v1.3.2`. The version without the prefix must match `version` in `custom_components/tsun_local/manifest.json`.
