...
Git tags use a `v` prefix. For example, release `1.3.3` uses tag `v1.3.3`. The version without the prefix must match `version` in `custom_components/tsun_local/manifest.json`.
...
6. Create and push the matching annotated tag:
   ...
   git switch main
   git pull --ff-only
   git tag -a v1.3.3 -m "Version 1.3.3"
