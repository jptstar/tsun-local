...
Git tags use a `v` prefix. For example, release `1.2.0` uses tag `v1.2.0`. The version without the prefix must match `version` in `custom_components/tsun_local/manifest.json`.
