...
def detect_pv_count(registers: dict[int, int]) -> int:
    """Return the PV input count advertised by register 0x1009."""
    count = registers.get(0x1009, 0)
    if not 1 <= count <= MAX_PV_COUNT:
        raise TsunProtocolError(
            f"Unexpected 1097 PV input count in register 0x1009: {count}"
        )
    return count
