...
版本 1.3.1 支持已在真实设备上验证的 **TSOL-MP3000** 和 **MX500**，并为其他等待验证的 **TITAN**、**GEN3** 和 **GEN3 PLUS** 型号提供支持。
...
3. 打开**设置 → 设备与服务 → 添加集成**。
4. 搜索 **TSUN Local**。
5. 输入 IP 地址和端口。Home Assistant 会尝试自动检测 **Monitor SN / Logger SN**。

添加设备时，请选择“**搜索本地网络**”或“**手动配置**”。Home Assistant 会先从 logger 的本地状态页面读取 **Monitor SN / Logger SN**。如果无法读取，表单允许手动输入页面或设备铭牌上的 **Device serial number**。集成会自动检测受支持的本地协议，无需选择设备系列。搜索会先通过 UDP/48899 发送只读的原生发现请求，并在所选 TCP 端口上验证每个响应。它还会自动复用每台已配置 TSUN 设备所在的 `/24` 子网。对于未知路由 VLAN 中的第一台设备，只需以 CIDR 格式输入一次子网；后续搜索会自动包含该子网。如果路由器阻止 VLAN 之间的广播，首次手动输入仍可能是必要的。
...
该集成创建一台设备，其中包含交流测量、每个已检测光伏输入的 5 项测量、已检测直流功率之和、4 项通信诊断、微型逆变器序列号、logger 固件版本和 MAC 地址诊断传感器，以及**微型逆变器在线**连接二进制传感器。
