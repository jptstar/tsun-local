...
版本 1.3.0 支持已在真实设备上验证的 **TSOL-MP3000** 和 **MX500**，并为其他等待验证的 **TITAN**、**GEN3** 和 **GEN3 PLUS** 型号提供支持。
...
添加设备时，请选择“**搜索本地网络**”或“**手动配置**”。Home Assistant 会先从 logger 的本地状态页面读取 **Monitor SN / Logger SN**。如果无法读取，表单允许手动输入页面或设备铭牌上的 **Device serial number**。集成会自动检测受支持的本地协议，无需选择设备系列。如果未找到设备，表单允许以 CIDR 格式输入可路由的 LAN 或 VLAN 子网。
...
该集成创建一台设备，其中包含交流测量、每个已检测光伏输入的 5 项测量、已检测直流功率之和、4 项通信诊断、logger 固件版本和 MAC 地址诊断传感器，以及**微型逆变器在线**连接二进制传感器。
