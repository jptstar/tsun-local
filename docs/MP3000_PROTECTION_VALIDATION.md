...
| Entity key | Home Assistant name | Proposed 1511 register | Proposed scaling | Exported reference value | Status |
|---|---|---:|---:|---:|---|
| `grid_qp_voltage_threshold` | QP voltage threshold | `0x07D2` | ×0.1 V | 105 V | ⚠️ Physical test required |
| `grid_recovery_speed` | Recovery speed | `0x07D3` | ×0.1 s | 640.0 s | ⚠️ Physical test required |
| `grid_overtemperature_protection_value` | Overtemperature protection value | `0x07D8` | ×1 °C | 79 °C | ⚠️ Physical test required |
| `grid_overfrequency_reduction_frequency` | Overfrequency reduction value | `0x07E1` | ×0.01 Hz | 50.20 Hz | ⚠️ Physical test required |
| `grid_overfrequency_reduction_coefficient` | Overfrequency reduction coefficient | `0x07EC` | raw | `0FA0` | ⚠️ Physical test required |
| `grid_start_upper_voltage` | Upper start voltage limit | `0x07F1` | ×0.1 V | 251.00 V | ⚠️ Physical test required |
| `grid_start_lower_voltage` | Lower start voltage limit | `0x07F2` | ×0.1 V | 196.00 V | ⚠️ Physical test required |
| `grid_start_upper_frequency` | Upper start frequency limit | `0x07F3` | ×0.01 Hz | 50.09 Hz | ⚠️ Physical test required |
| `grid_start_lower_frequency` | Lower start frequency limit | `0x07F4` | ×0.01 Hz | 49.51 Hz | ⚠️ Physical test required |
| `grid_connection_time` | Grid connection time | `0x07F7` | ×0.1 s | 40.0 s | ⚠️ Physical test required |
| `grid_reconnection_time` | Grid reconnection time | `0x07F8` | ×0.1 s | 40.0 s | ⚠️ Physical test required |
| `grid_ten_minute_overvoltage_protection` | 10-minute overvoltage protection | `0x07F9` | ×0.1 V | 253.00 V | ⚠️ Physical test required |
