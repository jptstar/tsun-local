...
class DiagnosticDesktopTestModeTests(unittest.TestCase):
    def test_magic_offsite_trigger_is_exact(self) -> None:
        self.assertEqual(desktop.legacy.MAGIC_TEST_HOST, "89:89:89:89")
        self.assertEqual(desktop.legacy.MAGIC_TEST_SN, "89898989")

    def test_test_report_is_explicitly_synthetic_and_read_only(self) -> None:
        source = Path(desktop.legacy.__file__).read_text(encoding="utf-8")
    ...
    def test_desktop_version_was_bumped(self) -> None:
        ...
        self.assertEqual(desktop.legacy.base.APP_VERSION, "1.5.8")
        self.assertEqual(desktop.legacy.upload_app.APP_VERSION, "1.5.8")
